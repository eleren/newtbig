...
func (s *service) register() error {
	...
	KAPI := s.app.GetKeysAPI()
	_, err = KAPI.Put(context.Background(), s.Key, string(data))
	if err != nil {
...
func (s *service) deregister() error {
	...
	KAPI := s.app.GetKeysAPI()
	if KAPI != nil {
		_, err := KAPI.Delete(context.Background(), s.Key, nil)
		if err != nil {
...
func (s *service) run(exit chan struct{}) {
	if s.app.Options().RegisterInterval <= 0 {
		log.Logger.Warn("service run  RegisterInterval error")
		return
	}
	...
	for {
		...
		case <-t.C:
			err := s.register()
			if err != nil {
				log.Logger.Warnf("service run register error:%s ", err.Error())
