...
func newOptions(opts ...module.Option) module.Options {
	...
	if err != nil {
		panic(fmt.Errorf("options read config error config file: %s \n", err))
	}

	logsPath := fmt.Sprintf("%s/%s", opt.WorkDir, viper.GetString("app.logPath"))
	_, err = os.Open(logsPath)
	if err != nil {
		err := os.Mkdir(logsPath, os.ModePerm) //
		if err != nil {
			panic(fmt.Errorf("options make logs dir err: %s \n", err))
		}
	}

	opt.ETCDHosts = viper.GetStringSlice("app.etcd.Hosts")
	if len(opt.ETCDHosts) == 0 {
		panic(fmt.Errorf("etcd config err: hosts is nil  \n"))
	}

	opt.NatsHosts = viper.GetStringSlice("app.nats.Hosts")
	if len(opt.NatsHosts) == 0 {
		panic(fmt.Errorf("nats config err: hosts is nil  \n"))
	}
