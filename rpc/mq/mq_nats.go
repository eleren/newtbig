...
func NewNatsClient(app module.App) (*NatsClient, error) {
	...
	if err1 != nil {
		log.Error("NewNatsClient JetStream nil err:%s ", err.Error())
		return nil, err1
	}
...
func (nc *NatsClient) CloseHandler(conn *nats.Conn) {
	log.Logger.Info("NatsClient nats_conn closed !")
}
...
func (nc *NatsClient) RequestMsg(subj string, msg *framepb.Msg) (*framepb.Msg, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("RequestMsg err :%s req :%d msgId :%d uid :%d  key:%d err:%s ", subj, msg.Seq, msg.ID, msg.UID, msg.Key, err.Error())
	}
	rst := utils.Get()
	err = proto.Unmarshal(msg1.Data, msg)
	if err != nil {
		return nil, err
	}
	return rst, nil
...
func (nc *NatsClient) StartPublishHandle() (err error) {
	...
	go func() {
		...
		case _, ok := <-nc.done:
			if !ok {
				log.Logger.Info("pub client  has done !")
			}
		}
		nc.subs.Unsubscribe()
	}()
	...
	for !nc.isClose {
		...
		if err != nil && err == nats.ErrTimeout {
			if !nc.subs.IsValid() {
				...
				if err != nil {
					log.Logger.Errorf("pub error: %s'", err.Error())
					continue
				}
			}
			continue
		} else if err != nil {
			log.Logger.Warnf("pub  error1 :%s", err.Error())
			if !nc.subs.IsValid() {
				nc.subs, err = nc.nats_conn.SubscribeSync(fmt.Sprintf("%s.*", nc.key))
				if err != nil {
					log.Logger.Errorf("pub error2 :%s ", err.Error())
					continue
				}
			}
			continue
		...
		if m == nil {
			log.Logger.Warnf("pub sub:%s receive is nil ", nc.key)
			continue
		}
		err = nc.handler(m)
		if err != nil {
			log.Logger.Errorf("pub handler sub:%s err :%s ", nc.key, err.Error())
		}
	}
	return nil
}
...
func (nc *NatsClient) StartRequestHandle() error {
	...
	subs, errS := nc.nats_conn.Subscribe(nc.reqKey, func(m *nats.Msg) {
		...
		if m != nil {
			...
			log.Logger.Error("request sub:%s  msg is nil ", m.Subject)
		}
	})
	if errS != nil {
